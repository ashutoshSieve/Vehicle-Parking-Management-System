# Vehicle Parking Management System (C++ | OOP | Industry-Style)
# Author -> Ashutosh Kumar (NIT Patna)

A backend simulation of a real-world parking lot:
- Assigns suitable parking slots to vehicles
- Tracks entry/exit with tickets
- Calculates fees using Strategy Pattern
- Clean OOP with extensibility in mind

> Built with C++17. types: `"Small"`, `"Medium"`, `"Large"`.

---

## Features

- Vehicle types: **Bike**, **Car**, **Truck**, **ElectricCar**
- Slot types: **Small**, **Medium**, **Large**
- String-based slot typing (no enums)
- Strategy Pattern for pricing (BikeFee, CarFee, TruckFee, ElectricCarFee)
- Facade: `ParkingLot` provides simple API
- Handles edge cases: full lot, invalid ticket, etc.
...
- **`Vehicle`** is a polymorphic base class; `Bike`, `Car`, `Truck`, `ElectricCar` derive from it.
- **`ParkingSlot`** keeps `slotId`, `occupied`, and `slotType` as a **string** (`"Small"|"Medium"|"Large"`).
- **`Ticket`** binds `Vehicle + Slot + entryTime`.
- **`FeeCalculator`** is an interface; concrete strategies apply per vehicle type.
- **`ParkingLot`** orchestrates:
  - slot allocation & freeing
  - ticket management
  - fee calculation on exit  
  It exposes a **simple API**: `park(vehicle)` and `unpark(ticketId)`.
